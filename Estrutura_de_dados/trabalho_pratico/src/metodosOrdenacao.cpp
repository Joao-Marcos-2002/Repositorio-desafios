...
void MetodoOrdenacao::inserctionSort(Vertice* vertices) {
    if (vertices == nullptr || vertices->proximo == nullptr) {
        return;  
    }

    Vertice* cabecaOrdenada = vertices; 
    vertices = vertices->proximo;  

    cabecaOrdenada->proximo = nullptr;  

    while (vertices != nullptr) {
        Vertice* chave = vertices;
        vertices = vertices->proximo;

        if (chave->cor < cabecaOrdenada->cor) {
            // Inserir no início da lista ordenada
            chave->proximo = cabecaOrdenada;
            cabecaOrdenada = chave;
        } else {
            // Encontrar a posição correta para inserção
            Vertice* atual = cabecaOrdenada;

            while (atual->proximo != nullptr && chave->cor > atual->proximo->cor) {
                atual = atual->proximo;
            }

            // Inserir após o 'atual'
            chave->proximo = atual->proximo;
            atual->proximo = chave;
        }
    }

    // Atualiza a cabeça da lista original
    vertices = cabecaOrdenada;
}
...
int MetodoOrdenacao::partition(Vertice* vertices, int esq, int dir) {
    ...
    while (true) {
        ...
        // Se os índices se cruzarem quer dizer que a partição está completa..
        if (i >= j) {
            return j;
        }
